...
<img width="1911" height="1072" alt="image" src="https://github.com/user-attachments/assets/503471ce-4698-4fba-b494-0e9bdd92e1ed" />
...
<img width="1907" height="1078" alt="image" src="https://github.com/user-attachments/assets/3c2194c2-9f75-4177-ba90-a6a0a0a1d779" />
...
<img width="1906" height="997" alt="image" src="https://github.com/user-attachments/assets/b97cd3d0-8734-42d2-8f57-a0a472d85448" />
...
<img width="1301" height="985" alt="image" src="https://github.com/user-attachments/assets/b5942f5c-df1c-4b6b-8978-37accae4e54d" />
...
4.<img width="1301" height="985" alt="image" src="https://github.com/user-attachments/assets/b5942f5c-df1c-4b6b-8978-37accae4e54d" />
...
```text
LayoutBuilder ใช้ดูพื้นที่ของ Widget ที่ได้รับจริง ส่วน MediaQuery ใช้ดูขนาดและข้อมูลของหน้าจอโดยรวม
```
2. ทำไม Go Router ถึงใช้ `StatefulShellRoute` แทน `ShellRoute` ธรรมดา? ผลต่างเรื่อง State Management คืออะไร?
```text
ใช้ StatefulShellRoute เพราะช่วยแยกและจำ State ของแต่ละ Tab เช่น Home หรือ Favorites ทำให้สลับ Tab แล้วหน้าที่เดิมยังอยู่
```
3. ในโค้ด `DestinationCard` เหตุใดจึงใช้ `Expanded` ครอบ `Text` ชื่อ Destination ? จะเกิดอะไรขึ้นถ้าลบออก?
```text
ใช้ Expanded เพื่อให้ Text ใช้พื้นที่ที่เหลือใน Row ถ้าลบออกข้อความยาวอาจล้นและเกิด Overflow ได้
```
4. การส่งข้อมูลผ่าน `extra` ของ Go Router มีข้อจำกัดอะไรกรณี Deep Link / Web Refresh? และแก้ปัญหานี้ได้อย่างไร?
```text
extra ไม่ถูกเก็บใน URL จึงอาจหายเมื่อ Deep Link หรือ Refresh และควรใช้ Path Parameter เช่น /destination/:id แทน
```
